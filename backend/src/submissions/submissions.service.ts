...
export class SubmissionsService {
  ...
  async submit(userId: string, questionId: string, submittedCode: string, executionTimeMs: number) {
    ...
    for (const qc of question.questionConcepts) {
        ...
        let pNew = pLearnedGivenEvidence + (1 - pLearnedGivenEvidence) * concept.pTransit;
    ...
    if (isCorrect) {
        ...
        languageProgress.proficiency = Math.min(3.0, languageProgress.proficiency + 0.05);
    } else {
    ...
    return { ...submission, output, userUpdates: { 
        xp: user?.xp, 
  ...
  private async handleCodingSubmission(question: Question, userCode: string): Promise<{ isCorrect: boolean, output: string }> {
      ...
      if (testCases.length === 0) {
           ...
           const result = await this.codeRunnerService.executeCode(question.language.name, userCode);
          ...
          if (langName === 'python') {
              ...
              if (!userCode.includes('class ')) {
                  ...
                  if (match) {
                      ...
                      if (typeof input === 'string' && input.startsWith("'") && input.endsWith("'") && input.length > 3) {
...
public class Main {
    public static void main(String[] args) {
        ...
        ${printStmt}
          ...
          const result = await this.codeRunnerService.executeCode(question.language.name, codeToRun);
